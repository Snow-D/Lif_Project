...
function showMarker(company, placeID, adress) {

    if(placeID != undefined || adress != undefined) {
        ...
        tabMarker.forEach(e => {
            if(company == e.id) {
                e.setIcon('http://maps.google.com/mapfiles/ms/icons/yellow-dot.png');
            }
        }); 
    }
...
$(function () {
    ...
    var trieRatingUp = $('#trieRatingUp');
    var trieRatingDown = $('#trieRatingDown');
    ...
    function initialRequestShowed(inputs) {
        ...
        fetch(`${server}/api/search`, {
            ...
            .then(json => {
                ...
                json.forEach(marker => {
                    infoWindow(map, addMarker(marker.position.lat, marker.position.lng, map, marker.company), json, marker.placeID, marker);
                });
            });
        ...
        fetch(`${server}/api/id`, {
            ...
            .then(
                fetch(`${server}/api/search`, {
                    ...
                    .then(r => r.json())
                    .then(json => {
                        if(filter.toString() == "trieRatingUp") {

                            json = json.filter(e => e.rating >= 3 && e.hasOwnProperty("position"));
                        }
                        else {
                            json = json.filter(e => e.rating <= 3 && e.hasOwnProperty("position"));
    ...
    trieRatingUp.on('click', function () {
        var filter = "trieRatingUp"
        filterByRating(filter);
    });

    trieRatingDown.on('click', function () {
        var filter = "trieRatingDown";
        filterByRating(filter);
    });
    ...
    function renderSearchIndeed(results, resSite) {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            results[i].company = results[i].company.replace(/\n/gi, ""); // Regex avec paramètre pour enlever plusieurs occurences
            ...
            if (results[i].site == "indeed") {
                ...
                rendu = `
                        ...
                        <h6 class="card-subtitle mb-2 text-muted">${results[i].company}</h6>
                        <p class="card-text">${results[i].summary}</p>
                        <a href="https://www.indeed.fr/voir-emploi?jk=${results[i].data_jk}" target="_blank" class="btn btn-outline-info">Postuler à l'annonce</a> 
                        ...
                        <button type="button" class="btn btn-outline-dark" data-toggle="popover" title="Info Entreprise" id="mapMarkerButton" onclick="showMarker('${results[i].company}', '${results[i].placeID}', '${results[i].adress}')">Info Map</button>
                        </div>
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            if (results[i].site == "linkedin") {                
                ...
                rendu = `
                        ...
                        <h6 class="card-subtitle mb-2 text-muted">${results[i].company}</h6>
                        <p class="card-text">${results[i].summary}</p>
                        <a href="https://fr.linkedin.com/jobs/view/${results[i].idOffer}" target="_blank" class="btn btn-outline-info">Postuler à l'annonce</a> 
    ...
    function addMarker(x, y, map, id, color) {
    ...
    function infoWindow(map, marker, parametreInfos, place_id) {
        ...
        infoMarker.addListener('closeclick', function () {
